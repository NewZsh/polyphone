...
def prepare_data(sent_path, lb_path=None, char2phonemes=None):
    raw_texts = open(sent_path).read().rstrip().split('\n')
    query_ids, texts = [], []
    for raw in raw_texts:
        tmp = raw
        ids = []
        while ANCHOR_CHAR in tmp:
            idx = tmp.index(ANCHOR_CHAR)
            ids.append(idx)
            if idx + 3 < len(tmp):
                tmp = tmp[:idx] + tmp[idx+1] + tmp[idx+3:]
            else:
    ...
    if lb_path is None:
        ...
        for line in open(lb_path):
            phonemes.append(line.strip().split())
...
class TextDataset(Dataset):
    ...
    def __init__(self, tokenizer, labels, char2phonemes, chars, texts, query_ids, phonemes=None, pos_tags=None,
                 use_mask=False, use_char_phoneme=False, use_pos=False, window_size=32, max_len=512, for_train=True):
    ...
    def _truncate_texts(self, window_size, texts, query_ids):
        ...
        for text, query_id in zip(texts, query_ids):
            for qid in query_id:
                start = max(0, qid - window_size // 2)
                end = min(len(text), qid + window_size // 2)
                truncated_text = text[start:end]
                truncated_texts.append(truncated_text)

                truncated_query_id = qid - start
