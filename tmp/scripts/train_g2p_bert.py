...
def test(config, checkpoint, device, sent_path=None, lb_path=None, pos_path=None):
    ...
    test_texts, test_query_ids, test_phonemes = prepare_data(sent_path, lb_path, char2phonemes)
...
def main(config_path):
    ...
    if config.aux_train_sent_path is not None:
        aux_train_texts, aux_train_query_ids, aux_train_phonemes = prepare_data(config.aux_train_sent_path, config.aux_train_lb_path)
        train_texts += aux_train_texts
        train_query_ids += aux_train_query_ids
        train_phonemes += aux_train_phonemes
    ...
    if config.use_pos and config.param_pos.get('aux_train_pos_path', '') != '':
        aux_train_pos_tags = prepare_pos(config.param_pos['aux_train_pos_path'])
        train_pos_tags += aux_train_pos_tags
